...
First, be sure to use the newest version of the library:

```{r}
#devtools::install_github('mikldk/malan')
```

Then, the library et al. are loaded:
```{r}
library(tidyverse)
library(malan)
library(ggraph)
...
This is rather difficult to make any sense of. Let's instead plot only pedigree 2:
