...
// There are count_father_haplotype_occurrences_pedigree matches. 
...
//' @export
// [[Rcpp::export]]
Rcpp::IntegerMatrix pedigree_father_haplotype_matches_in_pedigree_meiosis_L1_dists(const Rcpp::XPtr<Individual> suspect, int generation_upper_bound_in_result = -1) {
  const std::vector<int> h = suspect->get_father_haplotype();
  ...
  for (auto dest : *family) { 
    ...
    if (dest_h == h) {
      ...
      for (auto intermediate_node : path) { 
        ...
        int d = suspect->get_father_haplotype_L1(intermediate_node);
